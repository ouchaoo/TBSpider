...
class Tool{
    ...
    static ArrayList<String[]> analysisPost(String post){
        ...
        "[\\s\\S]*?" +
        "j_d_post_content  clearfix\">([\\s\\S]*?)</div><br></cc>?"
        );
        Pattern messagePa = Pattern.compile(messageRe);

        //提取帖子内容的正则表达式
        //String contentRe = new String("j_d_post_content  clearfix\">([\\s\\S]*?)</div><br></cc>?");
        //Pattern contentPa = Pattern.compile(contentRe);

        return analysis(post, messagePa);
    }
    ...
    static ArrayList<String[]> analysisThread(String homePage){
        ...
        "[\\s\\S]*?" +
        "target=\"_blank\" class=\"j_th_tit \">(.*?)</a>" +
        "[\\s\\S]*?" +
        "data-field='\\{&quot;user_id&quot;:(\\d*?)\\}'"
        ;
        Pattern metaMessagePa = Pattern.compile(metaMessageRe);

        return analysis(homePage, metaMessagePa);
     ...
     * @param  Pattern  pa        正则表达式列表
     * @return         匹配结果
     */
    static private ArrayList<String[]> analysis(String html, Pattern pa){
            ArrayList<String[]> result = new ArrayList<>();
